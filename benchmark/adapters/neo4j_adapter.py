"""
Neo4j Database Adapter (AuraDB Free and Neo4j Community)
Connects to Neo4j via official Bolt driver using Cypher.
"""
...
class Neo4jAdapter(BaseGraphAdapter):
    def __init__(self, name: str, config: Dict[str, Any], env_vars: Dict[str, str]):
        ...
        self.uri = env_vars.get("uri", "bolt://localhost:7687")
        self.user = env_vars.get("user", "neo4j")
        self.password = env_vars.get("password", "")
        self.database = env_vars.get("database", "neo4j")
        self.driver: Optional[Driver] = None

    def connect(self) -> bool:
        if not self.uri or not self.password:
            logger.warning(f"[{self.name}] Missing NEO4J_URI or NEO4J_PASSWORD environment variables.")
            return False
        try:
    ...
    def get_footprint(self) -> Dict[str, Any]:
        return {
            "tier": "AuraDB Free (or Community Container)",
            "limit_nodes": "200,000",
            "limit_relationships": "400,000",
